...
public class ProductController {
    ...
    @PostMapping
    private ResponseEntity<Product> savePlan (@RequestBody Product product) {
        
        Product tempProd = pService.create(product);

        try {

            return ResponseEntity.created(new URI("/api/plans"+tempProd.getId())).body(tempProd);
